document.addEventListener('DOMContentLoaded', async () => {
  const [battles, hqs] = await Promise.all([fetchJSON(UHPaths.batalhas), fetchJSON(UHPaths.hqs)]);
  document.getElementById('battles-grid').innerHTML = battles.map(battle => {
  ...
  document.getElementById('battle-search')?.addEventListener('input', event => {
